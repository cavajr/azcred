...
export class CorretorTabelaFiltro {
  banco_id: number;
  convenio_id: number;
  tipo_id: number;
  perfil_id: number;
...
export class CorretoresTabelasService {
  tabelaUrl: string;

  constructor(public http: SistemaHttp) {
    this.tabelaUrl = `${environment.apiUrl}/corretores/tabelas`;
  }

  pesquisar(filtro: CorretorTabelaFiltro): Promise<any> {
  ...
  exportar(filtro: CorretorTabelaFiltro) {
    let params = new HttpParams({});
